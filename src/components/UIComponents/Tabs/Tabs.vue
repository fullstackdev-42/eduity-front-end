      ...
      <ul class="nav"
        ...
        <li v-for="tab in tabs"
          ...
          <a data-toggle="tab"
             ...
             :class="{active: tab.active || tab.value === activeTab }">
            <tab-item-content :tab="tab" class="mx-5">
            </tab-item-content>
          </a>
  ...
  export default {
    ...
    methods: {
      findAndActivateTab(label){
        let tabToActivate = Object.keys(this.tabs).find(t=> t.label === label);
        if(tabToActivate){
      ...
      addTab(tab) {
        ...
        Object.keys(this.tabs).splice(index, 0, tab)
      },
      removeTab(tab) {
